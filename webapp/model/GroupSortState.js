sap.ui.define([
	...
	return BaseObject.extend("maxipoint.model.GroupSortState", {
		 ...
		 * @alias maxipoint.model.GroupSortState
		 */
		...
		/**
		 * Sorts by ShipName, or by OrderID
		 *
		...
		/**
		 * Groups by OrderID, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "OrderID") {
				...
				this._oViewModel.setProperty("/sortBy", "OrderID");

				aSorters.push(
					new Sorter("OrderID", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "ShipName");
			}
